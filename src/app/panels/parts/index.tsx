...
export const PartsPanel = ({ viewport, loading, isMobile, outliner }: Props) => {
    const [obj, setObj] = useState<Object3D | null>(viewport.selection?.object || null);
    ...
    return outliner.model && !isMobile ? (
        <Panel
            opened={viewport.selection ? true : false}
            title={`${outliner.model.name} Parts`}
                ...
                {outliner.model?.children?.map((item, i) => {
                    return (
                        <OutlinerButton
                            ...
                            id={`obj_${item.id}`}
                            key={i}
                            active={viewport.selection?.object?.id === item.id}
                            text={item.name}
                            onVisible={(visible, e) => {
